...
def get_portfolio_balance(portfolio_uuid):
    ...
    Εάν το ENABLE_DEMO_MODE είναι ενεργό, επιστρέφει δεδομένα επίδειξης.

    :param portfolio_uuid: Το μοναδικό ID του χαρτοφυλακίου.
    :return: Ένα λεξικό με το όνομα και το συνολικό υπόλοιπο σε μετρητά.
    """
    if ENABLE_DEMO_MODE:
        logging.info("Demo mode enabled. Returning demo portfolio balance.")
        return {
            "portfolio_name": "Demo Portfolio Name",
            "total_cash_equivalent_balance": 195000.00  
        }
...
def initialize_highest_price():
    """Initializes the highest price if not already set."""
...
def update_highest_price(current_price):
    """Updates the highest price if the current price exceeds it."""
    global highest_price
...
def check_stop_loss(current_price):
    """Checks and executes the stop-loss logic."""
    global active_trade, trade_amount, daily_profit, current_trades, highest_price

    stop_loss_price = calculate_stop_loss()
    if current_price <= stop_loss_price:
        logging.info(f"Stop-loss triggered. Attempting to sell at current price: {current_price}")
        execute_sell("Stop-Loss", current_price)


def calculate_stop_loss():
    """Calculates the stop-loss price based on configuration."""
...
def check_trailing_profit(current_price):
    """Checks and executes the trailing profit logic."""
    global trailing_profit_active

    scalp_target_price = active_trade * SCALP_TARGET
    if not trailing_profit_active and current_price >= scalp_target_price:
        logging.info("Scalp target reached. Trailing profit activated.")
...
def check_scalp_target(current_price):
    ...
    if current_price >= scalp_target_price and scalp_profit >= MINIMUM_PROFIT_THRESHOLD:
        execute_sell("Scalp Target", current_price)


def execute_sell(reason, current_price):
    """Executes a sell order and handles post-sell logic."""
    global active_trade, trade_amount, daily_profit, current_trades, highest_price, trailing_profit_active
    ...
    if order_successful and execution_price:
        ...
        sendgrid_email(trade_amount, "sell", execution_price, profit_loss, "N/A", reason)
        ...
        current_trades += 1
        save_state()
        save_cooldown_state(custom_duration=3600)
    else:
        logging.warning(f"Failed to execute sell order for {reason} at {current_price}")



def compute_indicators(df):
    """
    Υπολογίζει τους τεχνικούς δείκτες βάσει των δεδομένων.
    """
...
def validate_fetched_data(df, source_url):
...
def prepare_data(df):
    """Προετοιμασία δεδομένων για υπολογισμό δεικτών."""
...
def handle_sell_logic(current_price, indicators):
    """
    Διαχείριση λογικής πώλησης.
    """
    global highest_price

    logging.info(f"Active trade exists at {active_trade:.2f}. Checking for sell opportunity.")
    initialize_highest_price()
    update_highest_price(current_price)

    if ENABLE_STOP_LOSS:
        check_stop_loss(current_price, indicators)

    if ENABLE_TRAILING_PROFIT:
        check_trailing_profit(current_price)
    else:
        check_scalp_target(current_price)



def handle_buy_logic(current_price, indicators):
    """Διαχείριση λογικής αγοράς."""
    score = calculate_trade_score(indicators, current_price)
    evaluate_buy_signal(score, indicators, current_price)


def calculate_trade_score(indicators, current_price):
    """Υπολογισμός συνολικής βαθμολογίας για εμπορικό σήμα."""
    weights = load_weights(CRYPTO_NAME)
    score = 0
    scores = {
        "macd": weights['macd'] * (1 if indicators['macd'] > indicators['signal'] else -1),
        "rsi": weights['rsi'] * (1 if indicators['rsi'] < RSI_THRESHOLD else -1),
        "bollinger": (
            weights['bollinger'] * (1 if current_price <= indicators['bollinger_lower'] else -1)
...
def log_score_details(scores, indicators, current_price, score):
    """Καταγραφή των λεπτομερειών της βαθμολογίας."""
    if ENABLE_TABULATE_INDICATORS:
        table_data = [
            ["MACD", indicators['macd'], "MACD > Signal", scores['macd']],
            ["RSI", indicators['rsi'], "RSI < Threshold", scores['rsi']],
            ["Bollinger", current_price, "Price > Bollinger Upper", scores['bollinger']],
            ["VWAP", current_price, "Price > VWAP", scores['vwap']],
            ["Total Score", "", "", score],
        ]
        table = tabulate(table_data, headers=["Indicator", "Value", "Condition", "Score"], tablefmt="pretty")
        logging.info("\n" + table)


def evaluate_buy_signal(score, indicators, current_price):
    """Αξιολόγηση αν πρέπει να γίνει αγορά."""
...
def execute_buy_order(current_price, indicators, score):
    ...
    if "error" not in portfolio_summary:
        ...
        if TRADE_AMOUNT <= available_cash:
            ...
            if order_successful and execution_price:
                finalize_buy_order(execution_price, fees, indicators, score)
        else:
            logging.warning(f"Insufficient funds. Needed: {TRADE_AMOUNT:.2f}, Available: {available_cash:.2f}")


def finalize_buy_order(execution_price, fees, indicators, score):
    """Ολοκλήρωση εντολής αγοράς."""
    global active_trade, trade_amount, daily_profit, highest_price, current_trades
    ...
    current_trades += 1
    sendgrid_email(
        trade_amount, "buy", execution_price, fees, f"Trade signal score: {score:.2f}", str(indicators)
    )
    save_state()
...
def check_daily_limits():
    ...
    return daily_profit >= DAILY_PROFIT_TARGET or current_trades >= MAX_TRADES_PER_DAY


def stop_bot_due_to_limits():
    """Σταματάει το bot αν επιτευχθούν τα ημερήσια όρια."""
    global start_bot
    logging.info(f"Daily profit target or maximum trades reached. Stopping bot.")
    send_push_notification(f"Daily profit target or maximum trades reached for {CRYPTO_NAME}.")
    start_bot = False
    save_state()
...
def execute_scalping_trade(CRYPTO_SYMBOL):
    ...
    try:
        ...
        if not validate_fetched_data(df, source_url):
            return

        # Προετοιμασία δεδομένων
        prepare_data(df)

        # Έλεγχος επάρκειας δεδομένων για τον υπολογισμό δεικτών
        if not calculate_indicators(df, source_url, short_ma_period, long_ma_period):
            logging.warning(f"Insufficient data to calculate indicators from {source_url}.")
            return

        # Υπολογισμός δεικτών (πραγματικός υπολογισμός)
        indicators = compute_indicators(df)
        #logging.info(f"{indicators}")
       

        # Εάν υπάρχει ενεργή συναλλαγή, έλεγχος για πώληση
        if active_trade:
